...
import org.bitbucket.nanojava.descriptor.ISubstanceDescriptor;
import org.bitbucket.nanojava.io.Loader;
	...
	private static Map<String, ISubstanceDescriptor> getDescriptorsList(String desc) {
		...
		try {
			...
			if ("all".equals(desc)) {
				...
				String packageName = "org.bitbucket.nanojava.descriptor";
					...
					try {
						...
						while (enumeration.hasMoreElements()) {
							...
							if (jarEntry.toString().endsWith(".class")) {
								...
								if (!map.keySet()
										.contains(className.replace("org.bitbucket.nanojava.descriptor.", ""))) {
									...
									map.put(className.replace("org.bitbucket.nanojava.descriptor.", ""),
				...
				Class<?> descriptorClass = Class.forName("org.bitbucket.nanojava.descriptor." + desc);
			...
			System.out.println(
					"\nYou should provide a valid descriptor class from package org.bitbucket.nanojava.descriptor \n");
			printUsageAndExit();
